...
**总结：9 个原始修复全部正确生效，但引入了 2 个新的高严重度回归 (A: 编译错误, B: vma 未初始化)，以及暴露了 3 个预存的中等问题。建议优先修复 BUG-A 和 BUG-B。**我先抓最新的 git 提交和它改动的 hunks，再对当前 `wxjump.c` 做一次语法检查和剩余问题复核。
UpdateTodos: 4 items

**Command Executed:**
```bash
git -C "C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch" --no-pager log --decorate --oneline -n 6; git -C "C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch" rev-parse HEAD; git -C "C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch" --no-pager show --stat --summary -n 1
```

**Output:**
```
0709dd5 (HEAD -> main) fix(wxjump): 浜屾瀹¤淇5涓猙ug (2 HIGH + 3 MEDIUM)
54a4ebd fix(wxjump): 淇9椤筨ug (鍩轰簬5.10 GKI鍐呮牳婧愮爜楠岃瘉)
42512be feat: 娣诲姞 W^X 褰卞瓙鍐呭瓨 hook 妗嗘灦鍙婂弽妫€娴嬪伐鍏烽泦
94e5be9 (upstream/main, upstream/HEAD) fist policy
ea6cf2b test LZ4F
6418c06 directly execute apd
0709dd5572954f2001612706038105659c85ee42
commit 0709dd5572954f2001612706038105659c85ee42 (HEAD -> main)
Author: xiameng666 <120152563+xiameng666@users.noreply.github.com>
Date:   Wed Mar 11 21:34:11 2026 +0800

    fix(wxjump): 浜屾瀹¤淇5涓猙ug (2 HIGH + 3 MEDIUM)
    
    Bug-A [HIGH]: kfn___flush_tlb_range 鍑芥暟鎸囬拡澹版槑娈嬬己 鈥?琛ュ叏鍙傛暟鍒楄〃
    Bug-B [HIGH]: stale region goto 璺緞 vma 鏈垵濮嬪寲 鈥?鍒濆鍖栦负 NULL
    Bug-C [MEDIUM]: wxjump_exit 鑷棆閿佸唴 GFP_KERNEL 鍒嗛厤 鈥?鍏?unlock 鍐嶅垎閰?    Bug-D [MEDIUM]: cleanup_list 缂哄皯 NULL 妫€鏌?鈥?鍔犲け璐ユ彁鍓嶈繑鍥?    Bug-E [MEDIUM]: PTE 鍐欏叆涓?TLBI 闂寸己灏?dsb(ishst) 鈥?wxjump_flush_tlb_page 鍏ュ彛鍔犲睆闅?    
    鍚屾椂鎻愪氦: bug-audit-v1/v2 瀹¤鎶ュ憡, wxjump-tutorial 鏁欑▼鏂囨。
    
    Co-Authored-By: Oz <oz-agent@warp.dev>

 .claude/settings.local.json         |    7 +
 kpms/demo-hello/.idea/editor.xml    |    9 +
 kpms/demo-hello/.idea/misc.xml      |   13 +
 kpms/demo-hello/.idea/vcs.xml       |    6 +
 kpms/demo-hello/.idea/workspace.xml |   67 +
 kpms/wxjump/bug-audit-claude-v1.md  |  384 ++
 kpms/wxjump/bug-audit-claude-v2.md  |   71 +
 kpms/wxjump/bug-audit-codex5.4.md   | 8634 +++++++++++++++++++++++++++++++++++
 kpms/wxjump/wxjump-tutorial.md      |  734 +++
 kpms/wxjump/wxjump.c                |   12 +-
 kpms/wxshadow/wxshadow.kpm.i64      |  Bin 0 -> 805593 bytes
 11 files changed, 9934 insertions(+), 3 deletions(-)
 create mode 100644 .claude/settings.local.json
 create mode 100644 kpms/demo-hello/.idea/editor.xml
 create mode 100644 kpms/demo-hello/.idea/misc.xml
 create mode 100644 kpms/demo-hello/.idea/vcs.xml
 create mode 100644 kpms/demo-hello/.idea/workspace.xml
 create mode 100644 kpms/wxjump/bug-audit-claude-v1.md
 create mode 100644 kpms/wxjump/bug-audit-claude-v2.md
 create mode 100644 kpms/wxjump/bug-audit-codex5.4.md
 create mode 100644 kpms/wxjump/wxjump-tutorial.md
 create mode 100644 kpms/wxshadow/wxshadow.kpm.i64
fatal: unknown write failure on standard output
```

最新 HEAD 是 `0709dd5`，这次只改了 `wxjump.c` 很小一部分。我现在直接看这 12 行改动有没有把上轮发现的问题补全，并顺手复核剩余高风险点。
...
**Command Executed:**
```bash
git -C "C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch" --no-pager diff 54a4ebd 0709dd5 -- "kpms/wxjump/wxjump.c"
```

**Output:**
```
diff --git a/kpms/wxjump/wxjump.c b/kpms/wxjump/wxjump.c
index 67588ad..2525b33 100644
--- a/kpms/wxjump/wxjump.c
+++ b/kpms/wxjump/wxjump.c
...
@@ -198,6 +198,7 @@ static inline void wx_flush_icache(unsigned long addr)
 ...
 static void wxjump_flush_tlb_page(void *vma, unsigned long addr)
 {
+    dsb(ishst);  /* 纭繚 PTE 鍐欏叆瀵?TLB walker 鍙鍚庡啀鎵ц TLBI */
     if (kfn_flush_tlb_page) {
         ((void (*)(void *, unsigned long))kfn_flush_tlb_page)(vma, addr);
         return;
...
-    void *vma;
+    void *vma = NULL;
     uint8_t kbuf[64]; /* 瓒冲瀹圭撼 20 瀛楄妭鐨?MOVZ/MOVK/BR */
...
我再往下看时发现一个更本质的点：这次修了 `T1SZ/TG1`，但 `wxjump` 其它大量逻辑仍然把页大小硬编码成 4KB。也就是说，**“识别页粒度”被修了，不等于“非 4KB 页大小真正可用”**。
...
**Command Executed:**
```bash
Get-ChildItem -Path "C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64" -Recurse -File | Select-String -Pattern "CONFIG_ARM64_4K_PAGES|CONFIG_ARM64_16K_PAGES|CONFIG_ARM64_64K_PAGES" | ForEach-Object { "{0}:{1}:{2}" -f $_.Path, $_.LineNumber, $_.Line.Trim() }
```

**Output:**
```
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\kernel-pgtable.h:20:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\kernel-pgtable.h:125:#if defined(CONFIG_ARM64_4K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\kernel-pgtable.h:127:#elif defined(CONFIG_ARM64_16K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\kvm_arm.h:169:#ifdef CONFIG_ARM64_64K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\kvm_arm.h:174:#elif defined(CONFIG_ARM64_16K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\memory.h:156:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\pgtable.h:527:#if defined(CONFIG_ARM64_64K_PAGES) || CONFIG_PGTABLE_LEVELS < 3
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\processor.h:60:#if defined(CONFIG_ARM64_64K_PAGES) && defined(CONFIG_KUSER_HELPERS)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\processor.h:62:* With CONFIG_ARM64_64K_PAGES enabled, the last page is occupied
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\processor.h:68:#endif /* CONFIG_ARM64_64K_PAGES */
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\sparsemem.h:11:#if defined(CONFIG_ARM64_4K_PAGES) || defined(CONFIG_ARM64_16K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\sparsemem.h:15:#endif /* CONFIG_ARM64_4K_PAGES || CONFIG_ARM64_16K_PAGES */
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\sysreg.h:1028:#if defined(CONFIG_ARM64_4K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\sysreg.h:1031:#elif defined(CONFIG_ARM64_16K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\include\asm\sysreg.h:1034:#elif defined(CONFIG_ARM64_64K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\hugetlbpage.c:43:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\hugetlbpage.c:65:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\hugetlbpage.c:129:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\hugetlbpage.c:493:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\hugetlbpage.c:507:#ifdef CONFIG_ARM64_4K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\mmu.c:699:BUG_ON(!IS_ENABLED(CONFIG_ARM64_16K_PAGES));
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\mmu.c:1214:BUG_ON(!IS_ENABLED(CONFIG_ARM64_16K_PAGES));
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\mmu.c:1335:return IS_ENABLED(CONFIG_ARM64_4K_PAGES) &&
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\proc.S:23:#ifdef CONFIG_ARM64_64K_PAGES
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\proc.S:25:#elif defined(CONFIG_ARM64_16K_PAGES)
C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64\mm\proc.S:27:#else /* CONFIG_ARM64_4K_PAGES */
```

MarkAsCompleted: 4 items
看了，最新 HEAD 是 `0709dd5`。  
这次提交把我上轮指出的 5 个点基本都补上了：

- `kfn___flush_tlb_range` 声明修好了
- stale region 的 `vma` 未初始化修好了
- `wxjump_exit` 里锁内 `GFP_KERNEL` 分配修好了
- `cleanup_list` 判空加了
- PTE 写入前的 `dsb(ishst)` 也补了

我刚对当前 `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c` 做了 `clang -fsyntax-only`，**当前版本语法能过**。

但结合这次 commit，**还剩下几处明确问题**：

## 1. 还有一个高风险 cleanup 问题：写 fault 会 free 仍可能在映射中的 shadow page
问题链路：

- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c (434-458)`：写 fault 直接走 `wxjump_auto_cleanup`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c (399-421)`：`auto_cleanup` 清状态并 `free_pages(shadow_va)`，**但不恢复原始 PTE，也不 flush TLB**

如果写 fault 发生在 `STATE_SHADOW_X`，当前用户 PTE 仍可能指向 `shadow_pfn`，这时直接 free shadow page，有变成“页表还指着已释放页”的风险。

这个我仍然认为是 **HIGH**。

## 2. `RELEASE` 在恢复 PTE 失败时，仍然会把 shadow page 释放掉
位置：

- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c (919-941)`

这里 `release_pte` 只有在 `get_user_pte()` 成功时才恢复原始 PTE；  
但即使 `release_pte` 没拿到，后面仍然会：

- 清 `pi`
- `free_pages(shadow_va)`

也就是：**恢复映射失败 != 停止释放 shadow**。  
这还是一个生命周期不一致问题，严重度我给 **HIGH/MEDIUM 之间**。

## 3. `exit_mmap_before` 仍然只处理前 32 个 region
位置：

- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c (1004-1027)`

这里固定用了：

- `struct wx_region *regions[32];`

超过 32 个时，后面的 region 不会进入本次退出清理。  
这是明确的上限 bug，属于 **MEDIUM**。

## 4. 这次虽然修了 `T1SZ/TG1`，但代码整体仍然是 **4KB-only**
这点是我这轮最想提醒你的。

相关硬编码还在：

- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:51` `WX_PAGE_SIZE = 4096`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:64` `PAGE_MASK_4K`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:212` `tlbi_val = addr >> 12`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:284` `pte_idx = (addr >> 12) & 0x1FF`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:296` `pfn << 12`
- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c:321` page index `>> 12`

所以现在的状态是：

- **页表粒度“识别”修了**
- 但**非 4KB 页大小“实现”并没有真修好**

如果目标就是你这套 4KB GKI，这个问题可能暂时不炸；  
但如果这次 commit 的意图是“把 TG1/T1SZ 修完以后就兼容更多配置”，那**还远远不够**。

我会把这个算作 **MEDIUM，且属于设计层面的未完成修复**。

## 5. 一个次要但真实的可移植性风险
位置：

- `Documents/GitHub/无痕hook/KernelPatch/kpms/wxjump/wxjump.c (179-184)`

`wx_flush_dcache_va()` 把 cache line size 写死成了 `64`。  
很多 arm64 机器确实是 64B，但这不是通用保证。  
这更像 **LOW/待增强项**，不如前面 4 个严重。

## 非功能性问题
这次 commit 还混进了不少不该一起进主提交的东西：

- `.claude/settings.local.json`
- `kpms/demo-hello/.idea/`
- `kpms/wxshadow/wxshadow.kpm.i64`
- 几个很大的审计 markdown

这不是运行时 bug，但从提交质量上看，**建议拆提交**。

## 结论
结合最新 `0709dd5`：

- **上轮指出的 5 个提交级问题，基本都修了**
- **当前代码能过语法检查**
- 但**还剩 4 个我认为成立的问题**，其中前 2 个仍然偏重：
  1. write fault cleanup 可能 free 仍被映射的 shadow page
  2. release 恢复 PTE 失败仍 free shadow
  3. `exit_mmap_before` 只有 32 个 region 上限
  4. 非 4KB 支持其实并没真正修完

如果你要，我下一步可以直接给你出一版：

- **“按严重度排序的剩余 bug 清单”**
- 或者 **“最小修复 patch 方案”**，只列该怎么改，不动代码。