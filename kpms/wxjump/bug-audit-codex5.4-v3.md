- 我先抓最新 HEAD 和它相对上一个提交的 `wxjump.c` 改动，再跑一次语法检查和剩余风险复核。
  UpdateTodos: 4 items
  ...
  **Output:**
  ```
  ccfac6f (HEAD -> main) fix(wxjump): 淇 Codex 浜ゅ弶瀹¤鍙戠幇鐨?3 涓棶棰
  0709dd5 fix(wxjump): 浜屾瀹¤淇5涓猙ug (2 HIGH + 3 MEDIUM)
  ...
  ccfac6fc30a0dba7b89f12f367349d82a073a948
  commit ccfac6fc30a0dba7b89f12f367349d82a073a948 (HEAD -> main)
  Author: xiameng666 <120152563+xiameng666@users.noreply.github.com>
  Date:   Wed Mar 11 21:48:15 2026 +0800
  
      fix(wxjump): 淇 Codex 浜ゅ弶瀹¤鍙戠幇鐨?3 涓棶棰?    
      Fix 1 [MEDIUM-HIGH]: auto_cleanup 閲婃斁 shadow page 鍓嶅厛鎭㈠鍘熷 PTE
        - 閬垮厤 write fault 绛夊満鏅笅 PTE 鎮瀭鎸囧悜宸查噴鏀剧墿鐞嗛〉
        - 閫氳繃 mm 鐩存帴閬嶅巻椤佃〃鎭㈠, VMA 涓嶅彲鐢ㄦ椂鍥為€€鍏ㄥ眬 TLBI
      
      Fix 2 [MEDIUM]: RELEASE 鎭㈠ PTE 澶辫触鏃朵笉閲婃斁 shadow page
        - get_user_pte 澶辫触鏃朵繚鐣?shadow 鍜岀姸鎬? 閬垮厤鎮瀭鏄犲皠
        - 杈撳嚭 pr_warn 渚夸簬璇婃柇
      
      Fix 3 [LOW-MEDIUM]: exit_mmap 鍘婚櫎 32 region 纭笂闄?      - 鏀逛负 for(;;) 寰幆, 姣忚疆鏈€澶氭敹闆?32 涓? 鐩村埌璇?mm 鍏ㄩ儴娓呯悊
      
      Co-Authored-By: Oz <oz-agent@warp.dev>
  
   kpms/wxjump/wxjump.c | 129 +++++++++++++++++++++++++++++++--------------------
   1 file changed, 79 insertions(+), 50 deletions(-)
  fatal: unknown write failure on standard output
  ```

  最新 HEAD 是 `ccfac6f`，提交信息看起来正是针对我上轮提到的 3 个剩余点。现在我直接核对这次具体怎么改，并确认当前文件是否还能编过。

  **Command Executed:**
  ```bash
  git -C "C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch" --no-pager diff 0709dd5 ccfac6f -- "kpms/wxjump/wxjump.c"
  ```

  **Output:**
  ```
  diff --git a/kpms/wxjump/wxjump.c b/kpms/wxjump/wxjump.c
  index 2525b33..da39167 100644
  ...
  +        pi->orig_pte = 0;
           ...
           wx_spin_unlock();
  +
  +        /* 鍏堟仮澶嶅師濮?PTE 鍐嶉噴鏀?shadow page锛岄伩鍏嶆偓鍨?PTE 鎸囧悜宸查噴鏀鹃〉 */
  ...
  +            int pte_restored = 0;
               ...
               /* 绮剧‘鎭㈠鍘熷 PTE (淇濈暀 PXN 绛夋墍鏈夊師濮嬫爣蹇? */
               {
  ...
  -            wx_flush_icache(page_addr);  /* 娓呴櫎 shadow 鎸囦护鐨?I-cache 娈嬬暀 */
  ...
  -            pi->state = STATE_NONE;
  -            wx_spin_unlock();
  +            /* 浠呭湪 PTE 鎴愬姛鎭㈠鍚庢墠閲婃斁 shadow page锛岄伩鍏嶆偓鍨傛槧灏?*/
  +            if (pte_restored) {
  ...
  +            }
           ...
           /* 杩樻湁鍏朵粬 patch锛屽埛鏂?icache 浣挎仮澶嶇殑瀛楄妭鐢熸晥 */
  ...
  -            regions[count++] = r;
  +    /* 寰幆鏀堕泦锛屾瘡杞渶澶?32 涓紝鐩村埌璇?mm 鐨勬墍鏈?region 鍏ㄩ儴澶勭悊 */
  +    for (;;) {
  +        count = 0;
  +        wx_spin_lock();
  +        list_for_each_entry_safe(r, tmp, &region_list, list) {
  ...
  **C:\Users\24151\Documents\GitHub\无痕hook\KernelPatch\kpms\wxjump\wxjump.c**
  ...
  我再确认一个逻辑点：`auto_cleanup` 现在统一“恢复 orig_pte 再 free”，但它也被 `Mapping Changed` 和 `VMA Gone` 这两类路径复用。我要核对这些调用点是否会导致它在“不该恢复”的情况下把旧 PTE 写回去。
  ...
  **Command Executed:**
  ```bash
  Get-ChildItem -Path "C:\Users\24151\Documents\GitHub\vmware-shared-gki-source\common\arch\arm64" -Recurse -File | Select-String -Pattern "EXPORT_SYMBOL\(flush_tlb_page\)|EXPORT_SYMBOL_GPL\(flush_tlb_page\)|EXPORT_SYMBOL\(__flush_tlb_range\)|EXPORT_SYMBOL_GPL\(__flush_tlb_range\)" | ForEach-Object { "{0}:{1}:{2}" -f $_.Path, $_.LineNumber, $_.Line.Trim() }
  ```
  ...
  **Output:**
  ...
  看了，最新 HEAD 是 `ccfac6f`。  
  结论先说：

  - **这次提交把上轮我指出的 3 个问题基本都修到了**
  - **当前 `kpms/wxjump/wxjump.c` 能过 `clang -fsyntax-only`**
  - 但**还剩 3 个比较实在的问题 + 1 个低风险可移植性问题**

  ## 这次修对了的
  `ccfac6f` 这次确实修了：

  - `kfn___flush_tlb_range` 声明/编译问题
  - `auto_cleanup` 释放 shadow 前先尝试恢复 PTE
  - `RELEASE` 在 PTE 恢复失败时不再直接 free shadow
  - `exit_mmap_before` 去掉 32 个 region 的硬上限

  这些在当前 `wxjump.c` 里都能看到，而且语法检查通过了。

  ## 还剩的问题
  ### 1. `auto_cleanup` 现在会在 **Mapping Changed / VMA Gone** 路径里也强行写回旧 `orig_pte`
  这是我觉得现在最值得继续修的点。

  调用点：

  - `kpms/wxjump/wxjump.c (509-518)`：`Mapping Changed (read)`
  - `kpms/wxjump/wxjump.c (570-579)`：`Mapping Changed (exec)`
  - `kpms/wxjump/wxjump.c (510-513)` / `(571-574)`：`VMA Gone`

  实现：

  - `kpms/wxjump/wxjump.c (424-432)`

  问题在于：

  - `wxjump_validate_mapping()` 失败，本来就表示“当前 PTE 已经不是 orig/shadow 那套了”
  - 但 `wxjump_auto_cleanup()` 现在只要还能拿到一个有效 PTE，就会把 **旧的** `saved_orig_pte` 写回去

  这意味着在 **mapping 确实已经变了** 的情况下，它可能把一个**合法的新映射**又覆盖回旧映射。

  **我会把这个看成当前剩余的最高风险问题。**

  ### 2. `RELEASE` 在“没真正释放成功”时仍然返回成功
  位置：

  - `kpms/wxjump/wxjump.c (933-973)`

  现在逻辑是：

  - `get_user_pte()` 失败 → 打 `pr_warn`
  - 不释放 shadow，不清状态
  - 但最后还是 `return 0`

  也就是说，调用方会收到“release 成功”，但实际上：

  - patch 还在
  - shadow 还在
  - 状态也还在

  这不再是悬垂映射了，但变成了**语义错误**：**操作失败却返回成功**。

  ### 3. 非 4KB 页大小其实还是没真修好
  虽然 `TG1/T1SZ` 的识别修了，但实现里还是大量写死 4KB：

  - `kpms/wxjump/wxjump.c:51` `WX_PAGE_SIZE = 4096`
  - `kpms/wxjump/wxjump.c:64` `PAGE_MASK_4K`
  - `kpms/wxjump/wxjump.c:212` `addr >> 12`
  - `kpms/wxjump/wxjump.c:284` `pte_idx = (addr >> 12) & 0x1FF`
  - `kpms/wxjump/wxjump.c:296` `pfn << 12`
  - `kpms/wxjump/wxjump.c:321` page index `>> 12`
  ...
  如果你的目标始终就是 4KB Android GKI，这个是**潜在问题**；如果想泛化，那这个还没完成。

  ### 4. `wx_flush_dcache_va()` 仍把 cache line size 写死成 64
  位置：

  - `kpms/wxjump/wxjump.c (179-184)`

  这是低风险项，但确实不够稳。  
  在很多 arm64 机器上没事，但不是通用保证。

  ## 总结
  结合最新这次 `ccfac6f`：

  - **这次提交整体是对的，修复质量比前两次明显好**
  - **当前代码至少已经“能编过”了**
  - 但还没到“完全没问题”的程度

  我现在最建议继续盯的优先级是：

  1. **`auto_cleanup` 不要在 `Mapping Changed / VMA Gone` 路径盲目恢复旧 `orig_pte`**
  2. **`wxjump_do_release` 在 `pte_restored=0` 时不要返回成功**
  3. **如果要宣称支持非 4KB，得把所有 `>>12 / <<12 / 0xFFF / 4096` 一起清掉**
